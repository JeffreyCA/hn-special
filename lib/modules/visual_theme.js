...
HNSpecial.settings.registerModule("visual_theme", function () {
    ...
    if (container) {
      ...
      if (!content.textContent.trim().length) {
        ...
        while (
          ...
          !container.children[i].getElementsByClassName("yclinks").length
        ) {
          // Stop at the footer
      ...
      if (form && !isCommentPage) {
        ...
        _.toArray(form.getElementsByTagName("textarea")).forEach(function (textarea) {
          textarea.parentElement.parentElement.children[0].classList.add(
        ...
        if (topcolorInput) {
          topcolorInput.parentElement.appendChild(
            _.createElement("p", {
              content:
                "Default HN color: <pre>#ff6700</pre> — HN Embolden color: <pre>#ff8937</pre>.",
            })
          );
        }
      }
      ...
      if (isCommentPage) {
        ...
        if (tableContainer.childElementCount >= 4) {
          ...
          if (textContainer.textContent.trim().length) {
            var nodes = _.toArray(textContainer.childNodes).filter(function (node) {
      ...
      if (location.pathname.match(/^\/topcolors/)) {
        ...
        var topcolorContainer =
          _.$("img[src='s.gif']")[0].parentElement.parentElement.parentElement;
  ...
  function stripAttributes(data) {
    var matchElements = "body, table, tr, td, span, p, div, input";
    var set;
    ...
    set.forEach(function (elem) {
      ...
      for (var i = 0; i < attrs.length; i++) {
        ...
        if (["colspan", "class", "id", "type", "name", "value", "indent"].indexOf(attr) !== -1)
          continue;
        names.push(attr);
      }
  ...
  function replaceArrows() {
    ...
    var isDark = HNSpecial.settings.moduleEnabled("dark_theme");
    var upArrow = chrome.runtime.getURL(
      isDark ? "resources/arrow-up-white.svg" : "resources/arrow-up-black.svg"
    );
    var downArrow = chrome.runtime.getURL(
      isDark ? "resources/arrow-down-white.svg" : "resources/arrow-down-black.svg"
    );
    var arrowPicture;
