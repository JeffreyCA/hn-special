function Settings() {
  ...
  // Load the settings
  _.request(chrome.runtime.getURL("lib/defaults.json"), function (code) {
    ...
    if (
      self.currentSettings.visual_theme ||
      ...
      if (self.currentSettings.dark_theme) {
        document.documentElement.classList.add("hnspecial-theme-preload-dark");
      }
    }
    ...
    _.load(function () {
      try {
        document.getElementById("hnmain").children[0].children[0].children[0].bgColor = "#ffffff";
      } catch (e) {}
      ...
      document.documentElement.classList.remove("hnspecial-theme-preload");
      document.documentElement.classList.remove("hnspecial-theme-preload-dark");
    });
  });
}
...
Settings.prototype.buildMenu = function (added, requirements, permissions) {
  ...
  if (container) {
    try {
    document.getElementById("hnmain").children[0].children[0].children[0].style.removeProperty("background-color");
    } catch (e) {}
    ...
    keys.forEach(function (key) {
      var block = self.createSettingsBlock(
        key,
        self.currentSettings[key],
        added.indexOf(key) !== -1
      );
      map[key] = block;
    ...
    if (isFirstTime) {
      ...
      items.inner.children[1].innerHTML =
        "<strong>Welcome to HN Special!</strong> This is the settings panel. You can use it to enable or disable this extension's features. Make sure to apply the changes when you're done.";
    }
    ...
    if (added.length) {
      var count =
        added.length === 1
          ? "<strong>a new feature</strong> has been added"
          : "<strong>" + added.length + " new features</strong> have been added";
      var list =
        added.length === 1
          ? wrap(added[0])
          : added
              ...
              .join(", ") +
            " and " +
            wrap(added.slice(-1)[0]);

      items.inner.children[1].innerHTML =
        "<strong>Hey there!</strong> This is a quick notification to let you know that " +
        count +
        ": " +
        list +
        ".";
...
Settings.prototype.buildMenuFrame = function (container, showTip) {
  ...
  button.appendChild(
    _.createElement("img", {
      ...
      attributes: {
        src: chrome.runtime.getURL("resources/gear.svg"),
      },
    })
  );
    ...
    var iframe = _.createElement("iframe", {
      ...
      attributes: {
        ...
        src: chrome.runtime.getURL(
          "lib/extras/permissions.html#" + moduleName + "~" + themeModifier
        ),
      },
    });
  ...
  Object.keys(requirements).forEach(function (key) {
    if (map[key]) {
      ...
      subordinate.addEventListener("change", function () {
        if (this.checked) {
          // All mandatory checkboxes must be enabled too
          mandatory.forEach(function (current) {
